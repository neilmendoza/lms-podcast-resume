...
1. **Auto-resume** -- Monkey-patches the Podcast ProtocolHandler's `getNextTrack` to check for a cached playback position and automatically seek to it when no explicit start time is set.

2. **Favorites fix** -- On startup, scans `favorites.opml` and adds `parser="Slim::Plugin::Podcast::Parser"` to any favorites that match feeds subscribed in the Podcast plugin but are missing the parser attribute.
